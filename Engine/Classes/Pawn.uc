...
// ##nerf WES 
// Hit sound for scoring points
#exec AUDIO IMPORT FILE="g:\nerfres\scrptsnd\specialfx\htchar.wav" NAME="HitSound" GROUP="Generic"
...
var travel PointPickup	Pointpickup;	// bonus Point pickups.
...
//##nerf WES Sounds
// Yahoo Sound for player. The only reason why it's here is 
// because I could sub class the sound for male and female.
// If we are using generic sound for both male and female, this 
// could be put in NerfI.u.
//
var(Sounds) sound	Yahoo;
...
//##nerf WES & DSL
var byte RFnum;                 // Race flag Number for the arena race
var NavigationPoint SpawnSpot;  // where to respawn upon suit death (last flag)
var byte BallHolding;			// Scavenger hunt balls which you are holding
var byte BallDeposit;			// Scavenger hunt balls which you have deposited

replication
{
	// Variables the server should send to the client.
	reliable if( Role==ROLE_Authority )
		Weapon, Pointpickup, PlayerReplicationInfo, Health, bCanFly,
        RFnum, SpawnSpot, BallHolding, BallDeposit;		// NERF
...
// NERF - begin
function bool IsHoldingBall(int i)
{
	return (((BallHolding >> i) & 1) == 1);
}

function SetHoldingBall(int i, bool Val)
{
	BallHolding = BallHolding & (~(1 << i));		// clear the bit position
	if (Val)
		BallHolding = BallHolding | (1 << i);		// set the bit position
}

function bool HasDepositedBall(int i)
{
	return (((BallDeposit >> i) & 1) == 1);
}

function SetDepositBall(int i, bool Val)
{
	BallDeposit = BallDeposit & (~(1 << i));		// clear the bit position
	if (Val)
		BallDeposit = BallDeposit | (1 << i);		// set the bit position
}

function bool ShouldDropGoldBall()
{
	if (BallDeposit == 126)				// 01111110 (binary)
		return True;
	else
		return False;
}

function bool AllBallsDeposited()
{
	if (BallDeposit == 254)				// 11111110 (binary)
		return True;
	else
		return False;
}
...
// NERF:DSL -- here's where we jot down the level designers notion
// of where we should respawn upon suit death in an ArenaRace
function RegisterFlag( NavigationPoint ReLocation )
{
    SpawnSpot = ReLocation;
}
...
{
//log(self$" vanilla client restart");
	Velocity = vect(0,0,0);
...
//##nerf WES
// TossBall when killed
function TossBall(Inventory Inv)
{
	local vector X,Y,Z;
	local int i;
...
// Score System.
			if (!( Level.Game.bTeamGame &&
				(InstigatedBy.PlayerReplicationInfo.Team == PlayerReplicationInfo.Team)))
			{

				if (checkhitpos(hitlocation))
					InstigatedBy.PlayerReplicationInfo.Score += 100;
				else 
					InstigatedBy.PlayerReplicationInfo.Score += 25;

				if (InstigatedBy.bIsPlayer)
					PlaySound(sound'HitSound', SLOT_Misc,1);
			}
		}
...
// NERF:DSL: added for WES
function bool checkhitpos(Vector HitLoc)
{
    local vector bubloc;
    local HitPoint bub;
    ...
    if (Chesthit(HitLoc))
    {
       bubloc = Location;
       bubloc.Z +=100;
       bub = spawn(class 'HitPoint',Self,'', bubloc);

       if (bub != None)
           bub.gotostate('Flowing');
       return True;
    }
    else  return false;
}

function bool Chesthit(Vector HitLoc)
{
    local vector X,Y,Z, HitVec, HitVec2D;
    local float dotp;
    local vector bubloc;
    ...
    if ( HitLoc.Z - Location.Z > 0.3 * CollisionHeight )
    {
        if (dotp > 0) //then hit in front
			return True;
	}
	else return False;
...
//##nerf WES FIXME
// Need to do that in order to play the dying animation.
// May be we can do better than that. We will see.
/*
	function BeginState()
	{
		SetTimer(0.3, false);
...
Begin:
//##nerf WES 
	//log(class$ " Pawn state Dying got call at Begin");
	FinishAnim();
	Sleep(0.2);
	SetTimer(0.3, false);
}
