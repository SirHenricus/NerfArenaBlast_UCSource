...
function LogSuicide(Pawn Killed, name DamageType, Pawn Instigator)
{
	...
	if (Killed.Weapon != None)
		LogEventString(GetTimeStamp()$Chr(9)$"knock out"$Chr(9)$KilledID$Chr(9)$Killed.Weapon.ItemName$Chr(9)$DamageType$Chr(9)$InstigatorString);
	else
		LogEventString(GetTimeStamp()$Chr(9)$"knock out"$Chr(9)$KilledID$Chr(9)$"None"$Chr(9)$DamageType$Chr(9)$InstigatorString);
}
...
defaultproperties
{
     ...
     GameName=nerfarena
     GameCreator=Visionary Media, Inc.
     GameCreatorURL=http://www.vmisoft.com/
