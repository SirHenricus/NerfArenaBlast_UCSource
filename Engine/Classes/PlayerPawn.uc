...
//##nerf WES FIXME.
//28 levels Hardcoded
// The following valuables has to be encrypted for the final product.
var globalconfig float LevelHighScore[28];	// High Score for each level. 
var globalconfig byte LevelClear[28];		// Top 3 finished.
var globalconfig string LevelRank[28];		// Ranking with the highscore.
var globalconfig float TotalScore;			// accumulated score for the game.
var globalconfig byte	ArenaAccessLevel;	// How many Arena you can access.
// DSL
var globalconfig bool           bMatriculate;       // actually, bWinner
var globalconfig EAIGameType	LastGameType;		// wherefrom I just came ( used by ReadyRoom Tally board )
...
var globalconfig float DesiredFOV;		// NERF
//var float DefaultFOV;
var globalconfig float DefaultFOV;		// NERF
...
replication
{
	...
	reliable if( Role<ROLE_Authority )
		...
		SwitchLevel, SwitchCoopLevel, Kick, KillAll, Summon, ActivateTranslator, Admin, AdminLogin, AdminLogout, Typing;
	unreliable if( Role<ROLE_Authority )
...
/*
// this section is commented out of the build but
// left in as a possible template for user input routing
// The six Nerf_? functions declared below were bound to
// appropriate the keys in DEFUSER.INI
//============================================================================
//##nerf DSL
// keypress translators for various nerfarious purposes
// for sample usage, see MyCon in LabBots
// NOTE: when a MyCon ( or any son of PP carrying mc_ functions )
// is placed in a level, it needs to be tagged 'TheCon' and
// it needs to be the one and only
//
// right now these are only used for the LabBots, but I bet
// they wind up being pressing into service for  the Tutorial level.
//
function PlayerPawn FindTheCon()
{
    local PlayerPawn pp;
    local name n;

    n = 'TheCon';
    foreach allactors( class'PlayerPawn', pp, n )
        break;
    return pp;
}
function mc_Nerf_Y();
...
function mc_Nerf_P();
function mc_Nerf_H();
exec function Nerf_Y() { (FindTheCon()).mc_Nerf_Y(); }
exec function Nerf_U() { (FindTheCon()).mc_Nerf_U(); }
exec function Nerf_I() { (FindTheCon()).mc_Nerf_I(); }
exec function Nerf_O() { (FindTheCon()).mc_Nerf_O(); }
exec function Nerf_P() { (FindTheCon()).mc_Nerf_P(); }
exec function Nerf_H() { (FindTheCon()).mc_Nerf_H(); }
*/
// DSL -- end of nerfarious code section
//============================================================================
...
// DSL hardcoded exiter
function ServerReStartGame()
{
	if ( (Level.NetMode == NM_StandAlone) && (LevelClear[19] == 1) )
    {
        bMatriculate = true;
        SaveConfig();
        ConsoleCommand("exit");
...
{
//	log(class$ "WES:NET calling restartplayer in dying with netmode "$Level.NetMode);
	if ( Level.NetMode == NM_Client )
		return;
	if( Level.Game.RestartPlayer(self) )
	{
//		log(class$ "WES:NET Level.Game.RestartPlayer return True");
		ServerTimeStamp = 0;
		TimeMargin = 0;
		Enemy = None;
//		log(class$ "WES:NET Level.Game.StartPlayer Getting call");
		Level.Game.StartPlayer(self);
		if ( Mesh != None )
			PlayWaiting();
//		log(class$ "WES:NET ClientReStart Getting call");
		ClientReStart();
	}
	else
		log(self$" Restartplayer failed");
}
...
event PostBeginPlay()
{
	...
	DodgeClickTime = FMin(0.3, DodgeClickTime);
	EyeHeight = BaseEyeHeight;
	bNeverSwitchOnPickup = bNeverAutoSwitch;
...
function CalcBehindView(out vector CameraLocation, out rotator CameraRotation, float Dist)
{
	...
	View = vect(1,0,-0.5) >> CameraRotation;
...
// NERF:DSL handshake with Tallyboard
function SetLastGameType( EAIGameType gt )
{
	LastGameType = gt;
	SaveConfig();
}
...
ignores SeePlayer, HearNoise, KilledBy, Bump, HitWall, HeadZoneChange, FootZoneChange, ZoneChange, Falling, TakeDamage, PainTimer, Died, Suicide;
	...
	function EndState()
	{
//log( "DSL Endstate at "$Level.TimeSeconds );
//log( "DSL next state is "$NextState );
		if ( bMatriculate && Level.NetMode == NM_Standalone )
			ConsoleCommand("exit");
	}
...
log( "DSL Beginstate at "$Level.TimeSeconds );
		EndZoom();
		AnimRate = 0.0;
		SimAnim.Y = 0;
		bFire = 0;
...
// DSL - standalone mod
		...
		if (Level.NetMode == NM_StandAlone)
		{
			...
			{
//##nerf WES FIXME
// Will add more to the list if there's more level.

				case "PM-Amateur"	:	CheckScore(0);
										break;
				case "PM-Sequoia"	:	CheckScore(1);
										break;
				case "AR-Sequoia"	:	CheckScore(2);
										break;
				case "SH-Sequoia"	:	CheckScore(3);
										break;
				case "PM-Orbital"	:	CheckScore(4);
										break;
				case "AR-Orbital"	:	CheckScore(5);
										break;
				case "SH-Orbital"	:	CheckScore(6);
										break;
				case "PM-Barracuda"	:	CheckScore(7);
										break;
				case "AR-Barracuda"	:	CheckScore(8);
										break;
				case "SH-Barracuda"	:	CheckScore(9);
										break;
				case "PM-Asteroid"	:	CheckScore(10);
										break;
				case "AR-Asteroid"	:	CheckScore(11);
										break;
				case "SH-Asteroid"	:	CheckScore(12);
										break;
				case "PM-Sky"		:	CheckScore(13);
										break;
				case "AR-Sky"		:	CheckScore(14);
										break;
				case "SH-Sky"		:	CheckScore(15);
										break;
				case "PM-Luna"		:	CheckScore(16);
										break;
				case "AR-Luna"		:	CheckScore(17);
										break;
				case "SH-Luna"		:	CheckScore(18);
										break;
				case "PM-Champion"	:	CheckScore(19);
										break;
			}
// DSL adding lastlevel tracking

            if( InStr(CurrentLevel,"Tut") >= 0 )        // was that a tutorial?
                gt = AIType_None;                       // for now
            else if ( InStr(CurrentLevel,"MX") >= 0 )   // was that a bonus level?
                gt = AIType_None;                       // for now
            else
            {
    			CurrentLevel = Left( CurrentLevel, 3 );
    			switch(CurrentLevel)
    			{
    				case "PM-":	gt = AIType_PointMatch;		break;
    				case "AR-": gt = AIType_ArenaRace;		break;
    				case "SH-": gt = AIType_ScavengerHunt;	break;
    				default:	gt = AIType_None;			break;
    			}
            }
			LastGameType = gt;
//log( self$" set LGT to "$gt );
			Saveconfig();
		}
	}

	function CheckScore(int i)
	{
		local byte Levelcount, Pos, rank;
		local PlayerReplicationInfo PRI;

		Pos = 0;
		ForEach AllActors(class'PlayerReplicationInfo', PRI )
		{
			if ( PRI.Score > PlayerReplicationInfo.Score )
				Pos++;
		}

		if (PlayerReplicationInfo.score > LevelHighScore[i])
		{
			TotalScore = 0;
			LevelHighScore[i] = PlayerReplicationInfo.score;
			rank = pos+1;
			switch(rank % 10)
			{
				case 1:	LevelRank[i] = string(rank)$"st"; break;
				case 2:	LevelRank[i] = string(rank)$"nd"; break;
				case 3:	LevelRank[i] = string(rank)$"rd"; break;
				default: LevelRank[i] = string(rank)$"th"; break;
			}

//##nerf WES FIXME
// Hardcoded 28 levels
			for (levelcount=0; levelcount<28; levelcount++)
				TotalScore=TotalScore + LevelHighScore[levelcount];
		}

		if (((Level.Game.Difficulty < 2) && (Pos < 3)) ||
			((Level.Game.Difficulty > 1) && (Pos < 1)))	
			LevelClear[i] = 1;

		for (levelcount=0; levelcount<28; levelcount++)
		{
			if (LevelClear[levelcount] == 1)
				continue;
			else break;
		}

		ArenaAccessLevel = ((LevelCount-1)/3) + 1;

		if (LevelClear[0] == 0)
			ArenaAccessLevel = 0;

		if ((LevelClear[0] == 1) && (ArenaAccessLevel < 1))
			ArenaAccessLevel=1;
// DSL 
		bMatriculate = false;
        if ( i == 19 )          // championship level
        {
            if ( LevelClear[i] == 1 )       // did we win?
                bMatriculate = true;
        }
...
function int GetLastCompletedLevel()		// DWG
{
	local int i;

	// find last level player completed
	for (i = 0; i < ArrayCount(LevelClear); i++)
	{
		if (LevelClear[i] == 0)
			break;
	}
...
defaultproperties
{
     ...
     WeaponPriority(0)=SShot
     WeaponPriority(1)=Ballzoka
     WeaponPriority(2)=Turbofir
     WeaponPriority(3)=Scatter
     WeaponPriority(4)=MightyMo
     WeaponPriority(5)=Pulsator
     WeaponPriority(6)=TripleShot
     WeaponPriority(7)=Hyperst
     WeaponPriority(8)=Sidewind
     WeaponPriority(9)=Whomper
