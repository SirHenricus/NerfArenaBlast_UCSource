...
////////////////
//
// Nerf specific enumerations (DSL)
//
//  EBotAdvice
//  EBallEvent
//  ETauntType
...
//  EBotAlias
// 
//  EBotAdvice -- property of RacePoints (NavigationPoint)
//  retrieved and processed by NerfBots.
//
enum EBotAdvice
{
    ADV_None,
    ADV_Start,          // wait for countdown before running (not used)
    ADV_Jump,           // access JumpZ property to see how high
    ADV_Shoot,          // access TargetTag property to acquire target
    ADV_WaitForNext,    // wait at this point for next point to show up
    ADV_SwitchWeapon,   // use a particular weapon
    ADV_Inspect,        // inspect a mover's keyframe
    ADV_EndOfLap,       // decrement lap counter, see if you winned
	ADV_WaitForMover	// look before leap
};
// BallBlaster announcerments
enum EBallEvent
{
    BE_Slam,
    BE_Bull,
...
//
//
//  ETauntType -- category of taunts voiced by bots and avatars
//
enum ETauntType
{
    TAUNT_General,          // all the ones that don't fit below
    TAUNT_MissedMe,         // you couldn't hit hippopotamus in a bathtub
    TAUNT_GotYa,            // i managed to land one on you
    TAUNT_GotYaGood,        // i destroyed your power suit
    TAUNT_YouSlow,          // i just passed you up
    TAUNT_Gloat,            // i won
    TAUNT_GotMe             // oops
};
//
//
//  ETeamType -- eight teams of 4 bots (or players) each
//
enum ETeamType
{
    TEAM_Twisters,
...
//
//
//  EBotAlias -- GP for name and skin arrays, whatever ya need;
//  the above ETeamType iterates through this sequence by fours
//
enum EBotIndex
{
    AKA_Ted,            // Twisters
...
//
//  EVType -- how to map two voices onto four characters
//
enum EVType
{
	VT_Mute,
...
// this enumeration located here so it will be visible to
// both Info->NerfBotInfo and Info->GameInfo->DM, ->AR, ->SH, ->LR
// as well as playerpawn
enum EAIGameType
{
    AITYPE_None,
    AITYPE_PointMatch,
    AITYPE_ArenaRace,
    AITYPE_ScavengerHunt,
    AITYPE_LabRat
};


//
// end of Nerf specific bloc
//
////////////////
...
var actor				  Touching[4];   // List of touching actors.
...
////////////////
//
// Nerf specific additions (DWG)
//

final function TraceLog(coerce string Topic, int Depth, coerce string S)
{
	local string FilterTopic;
	local int FilterDepth;
	local int FilterDynamic;
	local playerpawn P;

	GetTrace(FilterTopic, FilterDepth, FilterDynamic);	// retreive current values

	//log("filter topic,depth,dynamic=" $ FilterTopic $ "," $ FilterDepth $ "," $ FilterDynamic, 'SPECIAL');
	//log("topic,depth,S=" $ Topic $ "," $ Depth $ "," $ S, 'SPECIAL');

	if ((FilterTopic == "*" || Topic == "*" || InStr(Topic, FilterTopic) >= 0 || InStr(S, FilterTopic) >= 0) && (FilterDepth >= Depth))
	{
		Log(Topic $ ": " $ S, 'TraceLog');

		if (PlayerPawn(Owner) != None)
			PlayerPawn(Owner).ClientMessage("***" $ Topic $ ": " $ S);
		else
		{
			// gotta do it the hard way
			foreach AllActors(class'PlayerPawn', P)
				if(NetConnection(P.Player) == None)		// filter out remote network players
					P.ClientMessage("***" $ Topic $ ": " $ S);
		}
	}
}
