...
state GetStatus 
{
	event ReceivedText( IpAddr Addr, string Text )
	{
		...
		do 
		{
			...
			{
				bOK = GetNextValue(In, Out, Value);
				if (Value ~= "DeathMatchGame")			Value = "Point Match Game";
				else if (Value ~= "ScavengerHuntGame")	Value = "Ball Blast Game";
				else if (Value ~= "ArenaRaceGame")		Value = "Speed Blase Game";
...
/*			else if(Value ~= "gamemode") // "openplaying"
			...
			else if(Value ~= "AdminEMail")
			{
				...
				// Log("#### "$Value$" nerf://"$Server.IP$":"$string(Server.GamePort)$" "$string(Server.NumPlayers)$" "$string(Server.MaxPlayers));
			}
...
//	AddRule(ServerAddressText, "nerf://"$Server.IP$":"$string(Server.GamePort));
