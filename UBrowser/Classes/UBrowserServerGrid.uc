...
{
	Server	= AddColumn(ServerName, 120);
	Ping	= AddColumn(PingName, 30);
	MapName	= AddColumn(MapNameName, 95);
	Players	= AddColumn(PlayersName, 35);
...
function JoinServer(UBrowserServerList Server)
{
	...
	{
		GetPlayerOwner().ClientTravel("nerf://"$Server.IP$":"$Server.GamePort$UBrowserServerListWindow(GetParent(class'UBrowserServerListWindow')).URLAppend, TRAVEL_Absolute, false);
