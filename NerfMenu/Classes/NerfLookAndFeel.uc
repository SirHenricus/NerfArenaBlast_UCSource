class NerfLookAndFeel expands UWindowLookAndFeel;
...
function FW_DrawWindowFrame(UWindowFramedWindow W, Canvas C)
{
	if(W.bStatusBar) 
	{
		if (NerfRootWindow(W.Root) == None)
			return;

//		if (NerfPDAMed(NerfRootWindow(W.Root).MainWindow.ClientArea).StatusWindow != None)
//			NerfPDAMed(NerfRootWindow(W.Root).MainWindow.ClientArea).StatusWindow.SetStatus(W.StatusBarText);		
//		if (NerfRootWindow(Root).PDAMainWindow().StatusWindow != None)
//			NerfRootWindow(Root).PDAMainWindow().StatusWindow.SetStatus(W.StatusBarText);		
		if (NerfPDA(NerfRootWindow(W.Root).MainWindow.ClientArea).StatusWindow != None)
			NerfPDA(NerfRootWindow(W.Root).MainWindow.ClientArea).StatusWindow.SetStatus(W.StatusBarText);		
	}
}
...
//	W.DrawMiscBevel(C, W.EditAreaDrawX, 0, W.EditBoxWidth, W.WinHeight, Misc, 2);
	W.DrawStretchedTexture(C, W.EditAreaDrawX, 0, W.EditBoxWidth, W.WinHeight, Texture'ButtonOn');
...
{
	W.DrawStretchedTexture(C, 4, 0, W.WinWidth, W.WinHeight, Texture'ButtonOff');
}
...
function ComboList_DrawItem(UWindowComboList Combo, Canvas C, float X, float Y, float W, float H, string Text, bool bSelected)
{
	...
	if(bSelected)
	{
		Combo.DrawStretchedTexture(C, X + 4, Y, W - 8, 16, Texture'ButtonOver');
		C.DrawColor.R = 0;
		C.DrawColor.G = 0;
		C.DrawColor.B = 0;
	}
...
defaultproperties
{
     DefaultTextColor=(R=248,G=199,B=104)
     OverSound=Sound'NerfRes.PDAOver'
     DownSound=Sound'NerfRes.PDADown'
     ...
     CloseBoxOffsetX=3
     CloseBoxOffsetY=5
     Active=Texture'NerfRes.Icons.NerfActiveFrame'
     Inactive=Texture'NerfRes.Icons.NerfInactiveFrame'
     ActiveS=Texture'NerfRes.Icons.NerfActiveFrameS'
     Misc=Texture'NerfRes.Icons.NerfMisc'
     FrameTL=(W=2,H=16)
     FrameT=(X=32,W=1)
