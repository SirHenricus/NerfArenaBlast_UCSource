...
// NerfBots.
//=============================================================================
class NerfBots extends Pawn
	abstract;

#exec AUDIO IMPORT FILE="g:\NerfRes\scrptsnd\genericfx\Gjumpdwn.WAV" NAME="Land1" GROUP="Generic"
...
//##nerf WES 
// DSL's valuable
var     float       RecommendedJumpZ;
var     bool        bReadyToRun;
var     name        OrderTag;           // tag of object referred to by orders
var     bool        bReserved;
...
// target practice (shooting winkies)
var Weapon          OldWeapon;
var name            OldState;
var actor           PointTarget;
...
//##nerf WES
// DSL's Vox valuable
var     VoxBox      myVox;              // a means of self-expression

// these need to be overloaded by child
function GetSkins() {}
...
function WiseCrack()
{
    local   float       tFlt;               // temps for picking wisecracks
    local   ETauntType  ttt;


    if ( myVox == None )
        return;             // can't say nothin'

    tFlt = FRand();
    ttt = TAUNT_GotYa;      // default epithet
    if ( tFlt < 0.33 )
        ttt = TAUNT_General;
    else if ( tFlt < 0.66 )
        ttt = TAUNT_Gloat;
    myVox.Speak( self, ttt );
}

// DSL --
// not-quite-a-kludge to get bots to shoot point targets
// a botbait spot calls this function which puts bot
// in the TargetPractice state after which all is returned
// to the way it was before
function ShootThatTarget(actor Target)
{
    if ( Target == None )
        return;

    OldWeapon = Weapon;
    if ( Weapon == None || ((Weapon != None) && (!Weapon.IsA('SShot'))) )
    {
        PendingWeapon = Weapon(FindInventoryType(class'SShot'));
        if ( PendingWeapon == None )
        {
// give bot a gun, somehow he lost his pea shooter
//log ( self$" somehow lost his SShot" );
        }
    ...
    GotoState('TargetPractice');
}

state TargetPractice
{
Begin:
//log( self$" in PointTargetPractice with PointTarget of "$PointTarget );
    OldAcceleration = Acceleration;
    OldRotation = Rotation;
    Acceleration = vect(0,0,0); //stop
//log( self$" in PointTargetPractice" );
    DesiredRotation = Rotator(PointTarget.Location - Location);
    PlayTurning();
//log( self$" in PointTargetPractice" );
FaceTarget:
    if (NeedToTurn(PointTarget.Location))
    {
        TurnToward(PointTarget);
        Goto('FaceTarget');
    }
    TweenToFighter(0.1);
    ...
    FinishAnim();
    Target = PointTarget;
    AdjustAim( Weapon.ProjectileSpeed, Location, 0, false, false );
    bFire = 1;
    bAltFire = 0;
    PlayFiring();
//log( self$" about to shoot "$PointTarget );
    Weapon.Fire(0.5);
    Sleep(0.75);        // enough for one, possibly two shots to get off
    HaltFiring();
    FinishAnim();
...
//log( self$" going back to "$OldState );
...
{
    //TraceLog(class, 10, "in PlayWalking()");
...
{
    //TraceLog(class, 10, "in PlayTurning()");
...
function PlayWaiting()
{
    ...
    //TraceLog(class, 10, "in PlayWaiting()");
...
{
    //TraceLog(class, 10, "in PlayCrawling()");
...
function PlayRising()
{
    //TraceLog(class, 10, "in PlayRising()");
...
{
    //TraceLog(class, 10, "in PlaySwimming()");
...
function PlayFiring()
{
    //TraceLog(class, 10, "in PlayFiring()");
...
// DSL -- FIXME -- we don't have animgroup names!
...
function PlayChallenge()
{
    ...
    //TraceLog(class, 10, "in PlayChallenge()");
...
function PlayFeignDeath()
{
    ...
    //TraceLog(class, 10, "in PlayFeignDeath()");
...
//-------------------------------------------------------------------------
// DSL -- TODO - Gut, Head, Left & Right hit alternate versions
//       to be evaluated and used/dropped
/*
function PlayGutHit(float tweentime)
{
    //TraceLog(class, 10, "in PlayGutHit(" $ tweentime $ ")");
...
function PlayHeadHit(float tweentime)
{
    //TraceLog(class, 10, "in PlayHeadHit(" $ tweentime $ ")");
...
function PlayLeftHit(float tweentime)
{
    //TraceLog(class, 10, "in PlayLeftHit(" $ tweentime $ ")");
...
function PlayRightHit(float tweentime)
{
    //TraceLog(class, 10, "in PlayRightHit(" $ tweentime $ ")");
...
function PlayDying(name DamageType, vector HitLoc)
{
    ...
    //TraceLog(class, 10, "in PlayDying(" $ DamageType $ ",...)");
    ...
    if ( ((DamageType == 'Decapitated') ||
         (HitLoc.Z - Location.Z > 0.6 * CollisionHeight))
         && !Level.Game.bVeryLowGore )
    {
        DamageType = 'Decapitated';
        if ( Level.NetMode != NM_Client )
        {
// DSL -- if we were the server, this is where we would spawn a carcass
        }
...
// DSL -- another nice spot for a carcass
        }
        else
...
function PlayInAir()
{
    //TraceLog(class, 10, "in PlayInAir()");
...
{
    //TraceLog(class, 10, "in PlayLanded(" $ impactVel $ ")");
    ...
    if ( impactVel > 0.17 )
        PlaySound(LandGrunt, SLOT_Talk, FMin(4, 5 * impactVel),
                false,1600,FRand()*0.4+0.8);
    if ( !FootRegion.Zone.bWaterZone && (impactVel > 0.01) )
        PlaySound(Land, SLOT_Interact, FClamp(4 * impactVel,0.2,4.5),
                false,1600, 1.0);
...
{
    //TraceLog(class, 10, "in PlayWeaponSwitch(" $ NewWeapon $ ")");
...
{
    //TraceLog(class, 10, "in TweenToWalking(" $ tweentime $ ")");
...
{
    //TraceLog(class, 10, "in TweenToRunning(" $ tweentime $ ")");
...
{
    //TraceLog(class, 10, "in TweenToWaiting(" $ tweentime $ ")");
...
{
    //TraceLog(class, 10, "in TweenToSwimming(" $ tweentime $ ")");
...
{
    //TraceLog(class, 10, "in TweenToFighter(" $ tweentime $ ")");
...
state GameEnded
{
	ignores SeePlayer, EnemyNotVisible, HearNoise, TakeDamage, Died, Bump, Trigger, HitWall, HeadZoneChange,
			FootZoneChange, ZoneChange, KilledBy, Falling, WarnTarget, PainTimer;

	function BeginState()
	{
	}
Begin:
//log(self$": GameEnded label" );
    CalmDown();
	LoopAnim('Breath2');
}
// for reference --
// state GameEnded as she was originally wrote by Polge
//
//state GameEnded
//{
...
// performance upon the podium
state TakeABow
{
	ignores SeePlayer, EnemyNotVisible, HearNoise, TakeDamage, Died, Bump, Trigger, HitWall, HeadZoneChange,
			FootZoneChange, ZoneChange, Falling, WarnTarget, PainTimer;
...
{
//	log(class$ " WES: bot adjustskill got call");
	if ( bWinner )
	{
		PlayerKills += 1;
...
simulated function PlayFootStep()
{
	...
	decision = FRand();
	if ( decision < 0.34 )
		step = Sound(DynamicLoadObject("NerfKids.stwalk1", class'Sound'));
	else if (decision < 0.67 )
		step = Sound(DynamicLoadObject("NerfKids.stwalk2", class'Sound'));
	else
		step = Sound(DynamicLoadObject("NerfKids.stwalk3", class'Sound'));
...
function CallForHelp()
{
	...
	for ( P=Level.PawnList; P!=None; P=P.NextPawn )
		if ( P.IsA('NerfBots') && (P.PlayerReplicationInfo.Team == PlayerReplicationInfo.Team) )
...
{
//log ( self$" NB wtdo for"$LikelyState );
	bFire = 0;
	bAltFire = 0;
	bReadyToAttack = false;
	Enemy = None;
...
function rotator AdjustToss(float projSpeed, vector projStart, int aimerror, bool leadTarget, bool warnTarget)
{
	...
	aimerror = aimerror * (4.8 - 0.5 * (skill + FRand()));	
...
function eAttitude AttitudeTo(Pawn Other)
{
	...
	else if (RelativeStrength(Other) > Aggressiveness)
		return ATTITUDE_Fear;
	else if (RelativeStrength(Other) == Aggressiveness)
	{
		if (Level.Game.Difficulty < 2)
			return ATTITUDE_Fear;
		else
...
function ReSetSkill()
{
	...
	{
		Health = 80;
		ReFireRate = 2.0 * Default.ReFireRate;
	}
	else
...
auto state StartUp
{
	function BeginState()
	{
//log ( self$" in NerfBots start state" );
		SetMovementPhysics(); 
...
state Roaming
{
	...
	function PickDestination(optional bool bNoCharge)
	{
		...
		if ( (EnemyPoints != None) && !EnemyPoints.bDeleteMe 
			&& (EnemyPoints.Owner == None) )
		{
			DroppedDist = VSize(EnemyPoints.Location - Location);
			if ( (DroppedDist < 800) && ActorReachable(EnemyPoints) )
			{
				BestWeight = EnemyPoints.BotDesireability(self); 		
				if ( BestWeight > 0.4 )
				{
					MoveTarget = EnemyPoints;
					EnemyPoints = None;
					return; 
				}
				BestInv = EnemyPoints;
...
SpecialNavig:
	if ( SpecialPause > 0.0 )
	{
		Disable('AnimEnd');
		Acceleration = vect(0,0,0);
//log(class$ " WES: PlayChallenge at Hunting");		
		PlayChallenge();
		Sleep(SpecialPause);
		SpecialPause = 0.0;
		Enable('AnimEnd');
...
ignores EnemyNotVisible; 
	...
	function rotator AdjustAim(float projSpeed, vector projStart, int aimerror, bool leadTarget, bool warnTarget)
	{
		...
		FireSpot = LastSeenPos;
		aimerror = aimerror * (2.5 * (4 - skill - FRand()));	
...
Begin:
	Acceleration = vect(0,0,0);
//log(class$ " WES: PlayChallenge at StakeOut");		
	PlayChallenge();
	TurnTo(LastSeenPos);
...
//log(class$ " WES: PlayChallenge at Falling");		
		PlayChallenge();
		FinishAnim();
	}
	TweenToFalling();
...
Challenge:
	...
	Acceleration = vect(0,0,0); //stop
	DesiredRotation = Rotator(Enemy.Location - Location);
//log(class$ " WES: PlayChallenge at RangeAttack");		
	PlayChallenge();
	FinishAnim();
	if ( bCrouching && !Region.Zone.bWaterZone )
		Sleep(0.8 + FRand());
...
//		SetTimer(0, false);
	...
	function EndState()
	{
		if ( Health <= 0 )
        {
//log(self$" trying to exit Dying state without dying");
			GotoState('Dying', 'TryAgain');
        }
	}
...
defaultproperties
{
     ...
     WaterStep=Sound'NerfI.Generic.LSplash'
     bStasis=False
     DrawType=DT_Mesh
