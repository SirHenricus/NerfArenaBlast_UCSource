...
// Integrated by Wezo
//=============================================================================
class NerfJoinGameMenu extends NerfLongMenu;
...
function bool ProcessSelection()
{
	...
	{
		SaveConfigs();
		ListenMenuClass = class<Menu>(DynamicLoadObject("Nerfi.NerfListenMenu", class'Class'));
		ChildMenu = spawn(ListenMenuClass, owner);
	}
	else if ( Selection == 2 )
	{
		SaveConfigs();
		ChildMenu = spawn(class'NerfFavoritesMenu', owner);
	}
	else if ( Selection == 5 )
		PlayerOwner.ConsoleCommand("GSPYLITE");// PlayerOwner.ConsoleCommand("start http://www.nerf.com/serverlist");
	if ( ChildMenu != None )
	{
		HUD(Owner).MainMenu = ChildMenu;
		ChildMenu.ParentMenu = self;
...
defaultproperties
{
     ...
     MenuList(5)=Go to the Nerf server list
     MenuTitle=JOIN GAME
}
