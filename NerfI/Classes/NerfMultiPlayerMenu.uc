...
// Integrated by Wezo
//=============================================================================
class NerfMultiPlayerMenu extends NerfLongMenu;
...
function bool ProcessSelection()
{
	...
	if ( Selection == 1 )
	{
		if(PlayerOwner.Player.Console.IsA('UBrowserConsole'))
		{
			...
			ChildMenu = spawn(class'NerfJoinGameMenu', owner);
	...
	{
		if(PlayerOwner.Player.Console.IsA('UBrowserConsole'))
			ChildMenu = spawn(class'NerfMultiplayerMeshMenu', owner);
		else
			ChildMenu = spawn(class'NerfPlayerMenu', owner);
	}
