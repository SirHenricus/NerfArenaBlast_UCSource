...
function InitConfig(GameInfo G)
{
	...
	SvMenu = NerfServerMenu(ParentMenu.ParentMenu);
...
function bool ProcessSelection()
{
	...
	{
		ChildMenu = spawn(class'NerfIndivBotMenu', owner);
		NerfIndivBotMenu(ChildMenu).InitConfig(GameType);
	}
	else if ( Selection == 6 )
		DeathMatchGame(GameType).bMultiPlayerBots = !DeathMatchGame(GameType).bMultiPlayerBots;		
	else
