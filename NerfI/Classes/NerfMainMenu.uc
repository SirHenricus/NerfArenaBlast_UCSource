...
// Integrated by Wezo
//=============================================================================
class NerfMainMenu extends NerfLongMenu;
...
function BeginPlay()
{
	...
	while ( OrderMenuName != "" )
	{
		if ( OrderMenuName ~= "NerfiOrder.NerfOrderMenu" )
		{
			bShareware = true;
...
function bool ProcessSelection()
{
	...
	if ( Selection == 1 )
		ChildMenu = spawn(class'NerfGameMenu', owner);
	else if ( Selection == 2 )
		ChildMenu = spawn(class'NerfMultiPlayerMenu', owner);
	else if ( Selection == 3 )
		ChildMenu = spawn(class'NerfOptionsMenu', owner);
	else if ( Selection == 4 )
		ChildMenu = spawn(class'NerfVideoMenu', owner);
	else if( (Selection==5) && bShareware )
		ChildMenu = spawn(class<menu>(DynamicLoadObject("NerfiOrder.NerfOrderMenu",class'Class')), owner);		
	else	
		ChildMenu = spawn(class'NerfQuitMenu', owner);
...
defaultproperties
{
     ...
     HelpMessage(7)=Call TOLL FREE 1-888-For-Nerf to Order NERF ARENA!
     MenuList(1)=GAME
